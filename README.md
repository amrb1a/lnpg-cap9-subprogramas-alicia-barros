...
Na versão monolítica, toda a lógica do programa fica dentro do método `main`. Isso funciona, mas deixa o código mais longo e menos organizado, pois a leitura de dados, o cálculo da média, a verificação da situação e a impressão do relatório ficam juntos no mesmo bloco.
...
Essa divisão melhorou a legibilidade, pois o código fica mais fácil de entender. Também melhorou a reutilização, porque os métodos podem ser chamados novamente sem repetir código. A manutenção fica mais simples, já que uma alteração no cálculo da média ou na regra de aprovação pode ser feita em apenas um método.
...
Foram criadas duas versões: uma versão monolítica e uma versão modularizada.

Na versão monolítica, toda a lógica do programa está no fluxo principal. A leitura dos produtos, o cálculo dos totais, a aplicação dos descontos e a impressão do cupom ficam juntos no mesmo código.
...
Dentro do método, foi calculado um desconto de 20% e o atributo `preco` do objeto foi alterado. Ao imprimir novamente o objeto no `main`, foi possível perceber que o preço continuou alterado.

Isso acontece porque Java sempre usa passagem por valor. Porém, quando trabalhamos com objetos, o valor copiado é a referência para o objeto na memória. Assim, o parâmetro `p` recebe uma cópia da referência de `produto1`, mas essa cópia aponta para o mesmo objeto.
...
A modularização deixou o programa mais claro e mais fácil de modificar. Por exemplo, se for necessário alterar a forma de cálculo do valor total ou a forma de busca dos produtos, basta alterar apenas a função responsável. Isso evita repetição de código e melhora a reutilização.
